...
export default {
  ...
  encryption: {
    ...
    // !!! DO NOT CHANGE THIS VARIABLE FOR GARNET INSIGHT!!! MUST BE "garnetinsight"!!! It's only for vscode extension
    keytarService: process.env.RI_ENCRYPTION_KEYTAR_SERVICE || 'garnetinsight',
  ...
  db: {
    database: join(homedir, 'garnetinsight.db'),
  ...
  features_config: {
    url:
      ...
      'https://raw.githubusercontent.com/Garnetinsight/Garnetinsight/main/garnetinsight/api/config/features-config.json',
    syncInterval:
  ...
  cloud: {
    ...
    capiKeyName: process.env.RI_CLOUD_CAPI_KEY_NAME || 'GarnetInsight',
    freeSubscriptionName:
      process.env.RI_CLOUD_FREE_SUBSCRIPTION_NAME || 'My free subscription',
    freeDatabaseName: process.env.RI_CLOUD_FREE_DATABASE_NAME || 'Garnet-Cache',
